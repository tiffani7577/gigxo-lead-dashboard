...
export default function AdminAVRequests() {
  ...
  const { data: requests = [], refetch: refetchRequests, isLoading: loadingRequests } =
    trpc.admin.getAVRequests.useQuery(undefined, {
      enabled: user?.role === "admin",
    });

  const { data: workers = [], isLoading: loadingWorkers } = trpc.admin.getAVWorkers.useQuery(undefined, {
    enabled: user?.role === "admin",
  });
          ...
          {tab === "requests" ? (
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {!loadingRequests && requests.length === 0 ? <p className="text-sm text-slate-500">No AV requests yet.</p> : null}
                {requests.map((r: any) => {
                  ...
                  return (
                    <div key={r.id} className="border rounded-lg p-3 bg-white">
                      <div className="grid grid-cols-1 md:grid-cols-8 gap-2 text-sm">
                        <div><span className="font-medium">Company/Event:</span> {company || r.title}</div>
                        <div><span className="font-medium">Date:</span> {r.eventDate ? new Date(r.eventDate).toLocaleDateString() : "-"}</div>
                        <div><span className="font-medium">Location:</span> {r.location}</div>
                        <div><span className="font-medium">Roles:</span> {roles || "-"}</div>
                        <div><span className="font-medium">Urgency:</span> {urgency || "-"}</div>
                        <div><span className="font-medium">Contact:</span> {contact}</div>
                        <div><span className="font-medium">Pay Rate:</span> {payRate || "-"}</div>
                        <div>
                          <Badge variant="secondary">{status.replace("_", " ")}</Badge>
                        </div>
                      </div>
                      <div className="mt-3 flex gap-2 flex-wrap">
                        <Button size="sm" variant={status === "new" ? "default" : "outline"} onClick={() => updateStatus.mutate({ id: r.id, status: "new" })}>New</Button>
                        <Button size="sm" variant={status === "in_progress" ? "default" : "outline"} onClick={() => updateStatus.mutate({ id: r.id, status: "in_progress" })}>In Progress</Button>
                        <Button size="sm" variant={status === "fulfilled" ? "default" : "outline"} onClick={() => updateStatus.mutate({ id: r.id, status: "fulfilled" })}>Fulfilled</Button>
                        <Button size="sm" variant="ghost" onClick={() => setExpandedId(expandedId === r.id ? null : r.id)}>
                          {expandedId === r.id ? "Hide Details" : "Expand Details"}
                        </Button>
                      </div>
                      {expandedId === r.id ? (
                        <pre className="mt-3 text-xs bg-slate-50 border rounded p-3 whitespace-pre-wrap">{r.description}</pre>
                      ) : null}
                    </div>
                  );
                })}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {!loadingWorkers && filteredWorkers.length === 0 ? <p className="text-sm text-slate-500">No AV workers yet.</p> : null}
                {filteredWorkers.map((w: any) => {
                  ...
                  return (
                    <div key={w.id} className="border rounded-lg p-3 bg-white grid grid-cols-1 md:grid-cols-6 gap-2 text-sm">
                      <div><span className="font-medium">Name:</span> {w.name}</div>
                      <div><span className="font-medium">Phone:</span> {w.phone}</div>
                      <div><span className="font-medium">City:</span> {w.city}</div>
                      <div><span className="font-medium">Skills:</span> {skillsText}</div>
                      <div><span className="font-medium">Min Rate:</span> {w.minDayRate ?? "-"}</div>
                      <div><span className="font-medium">Same Day:</span> {w.availableSameDay ? "Yes" : "No"}</div>
                    </div>
                  );
                })}
              </CardContent>
